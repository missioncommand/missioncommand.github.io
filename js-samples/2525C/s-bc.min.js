...
var buffer=null;if(typeof OffscreenCanvas!=='undefined')
{buffer=new OffscreenCanvas(iwidth,iheight);}
var ctx=buffer.getContext('2d');ctx.drawImage(this.getImage(),x,y);return buffer;};armyc2.c2sd.renderer.utilities.ImageInfo.prototype.getCenteredImage=function(){var image=this._canvas,bi=null,x=0,y=0,height=image.height,width=image.width,point=this._center;try
...
{bi=new OffscreenCanvas(iwidth,iheight);}
else
{bi=document.createElement('canvas');bi.width=iwidth;bi.height=iheight;}
var ctx=bi.getContext('2d');ctx.drawImage(image,x,y);}
catch(err)
...
{width=((width-point.getX())*2);x=((width/2)-point.getX());}
if(typeof OffscreenCanvas!=='undefined')
{bi=new OffscreenCanvas(width,height);}
else
{bi=document.createElement('canvas');bi.width=width;bi.height=height;}
var ctx=bi.getContext('2d');ctx.drawImage(image,x,y);var center=point.clone();center.shift(x,y);var symbolBounds=this._symbolBounds.clone();symbolBounds.shift(x,y);var bounds=new armyc2.s2sd.renderer.so.Rectangle(0,0,width,height);ii=new armyc2.c2sd.renderer.utilities.ImageInfo(bi,center,symbolBounds,bounds);}
catch(err)
{armyc2.c2sd.renderer.utilities.ErrorLogger.LogException("ImageInfo","getCenteredImage",err);}
...
{_TwoLabelOnly=TwoLabelOnly;},setOverscanScale:function(overscanScale){_overscanScale=Math.max(overscanScale,1);},getOverscanScale:function(){return _overscanScale;},getFriendlyUnitFillColor:function()
...
{var buffer=_document.createElement('canvas');buffer.width=width;buffer.height=height;}
return buffer;},getYPositionForSCC:function(symbolID)
...
{if(typeof OffscreenCanvas!=="undefined")
{buffer=new OffscreenCanvas(image.width,image.height);}
else
{buffer=document.createElement('canvas');buffer.width=image.width;buffer.height=image.height;}
ctx=buffer.getContext('2d');ctx.drawImage(image,0,0);var ii=rcwl(buffer,symbolID,modifiers);callback(ii);};image.src=url;}};}());(function($){$.fn.render2525=function(){var msa=armyc2.c2sd.renderer.utilities.MilStdAttributes;var RS=armyc2.c2sd.renderer.utilities.RendererSettings;RS.setSinglePointSymbolOutlineWidth(0);function sanitize(symbolID){var code=symbolID.substring(0);var affiliation=symbolID.charAt(1);if(affiliation==="F"||affiliation==="H"||affiliation==="U"||affiliation==="N")
{}
